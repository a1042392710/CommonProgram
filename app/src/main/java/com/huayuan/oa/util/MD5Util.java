...
public class MD5Util {
    ...
    /**
     * 下面这些S11-S44实际上是一个4*4的矩阵，在原始的C实现中是用#define 实现的，这里把
     * 它们实现成为static final是表示了只读，切能在同一个进程空间内的多个 Instance间共享
     */
    ...
    /**
     * getkeyBeanofStr()是类keyBean最主要的公共方法，入口参数是你想要进行keyBean变换
     * 的字符串，返回的是变换完的结果，这个结果是从公共成员digestHexStr取得的．
     */
    ...
    public static MD5Util mBean = null;
    ...
    /**
     * F, G, H ,I 是4个基本的keyBean函数，在原始的keyBean的C实现中，由于它们是 简单的位运算，
     * 可能出于效率的考虑把它们实现成了宏，在java中，我们把它们 实现成了private方法，名字保持了原来C中的。
     */
